...
app.get('/fetch', async (req, res) => {
  try {
    console.log('[app] peticion saliente con axios 0.21.0');
    const r = await axios.get('https://api.github.com/zen', { timeout: 5000 });
    res.json({ ok: true, dato: r.data });
  } catch (err) {
    console.error('[app] error en la peticion con axios:', err.message);
...
app.get('/crash', (req, res) => {
  ...
  console.error('[app] el proceso terminara con codigo 1');
